...
class AISuite extends FunSuite with BeforeAndAfter {
  ...
  implicit val players: Players = (new Player1 with Human, new Player2 with Computer)
  ...
  test("search: should return move (0, 0)") {
    ...
    val board = new Board(repr)
      assert(AI.search(board, players._2) === (0, 0))
  }
  ...
  test("search: should return move (1, 1)") {
    repr(0)(0) = 1
    val board = new Board(repr)
    assert(AI.search(board, players._2) === (1, 1))
  }
  ...
  test("search: should return a corner move") {
    val board = new Board(repr)
    val move = AI.search(board, players._2)
  ...
  test("search: should pick winning move") {
    ...
    repr(1)(1) = 1
    val board = new Board(repr)
    assert(AI.search(board, players._2) === (0, 2))
  }
}
